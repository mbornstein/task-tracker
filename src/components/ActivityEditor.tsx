...
export const ActivityEditor: React.FC<IActivityEditorProps> = ({ activity, onChangeActivity, projects, types }) => {
    const onChangeValueFor = React.useCallback(
        <T, Field extends keyof Activity>(field: Field, extract: (value: T) => Activity[Field]) =>
            (event: T) =>
    ...
    return (
        <div className={cx(css.margin, css.gap, css.row, css.wrap)}>
            <InputGroup
                ...
                onChange={onChangeValueFor("title", event => event.target.value)}
            />
            <InputGroup
                className={css.grow}
                placeholder="Comment"
                value={activity.comment || ""}
                onChange={onChangeValueFor("comment", event => event.target.value)}
            />
            ...
            <div className={css.grow}>
                <CategorySelect onChangeValue={onChangeValueFor("type", x => x)} value={activity.type} items={types} />
            </div>
            <InputGroup
                className={css.grow}
                placeholder="Link"
