                        ...
                        halves.map((half, iHalf) =>
                            tens.map((ten, iTen) => (
                                <tr
                                    ...
                                    key={`${hour}-${half}-${ten}`}
                                >
                                    <td className={css.fixedwidth}>
                                        {iHalf === 0 && iTen === 0
                                            ? hour
                                            : undefined}
                                    </td>
                                    <td className={css.fixedwidth}>
                                        {iTen === 0 ? half : undefined}
                                    </td>
                                    {[...Array(numDays).keys()].map(d => {
                                        const truncatedCellDateTime =
                                            addSecondsToDate(
                                                truncDay(firstDay),
                                                getOffsetSeconds(
                                                    d - numDays + 1,
                                        ...
                                        const activity = timeToActivityMap.get(
                                            truncatedCellDateTime.toISOString(),
                                        )
                                        const className = cx(
                                            css.cell,
                                            truncatedCellDateTime.getTime() ===
                                                truncateToCell(
                                                    new Date(),
                                                ).getTime()
                                                ? css.today
